...
namespace IFlair\AdvancedImportExport\Model\Export;
...
class Product extends \Magento\ImportExport\Model\Export\Entity\AbstractEntity
{
    ...
    {
        $helper = $this->_objectManager->create('IFlair\AdvancedImportExport\Helper\Data');
        if($helper->getSystemConfigValue('enable')) {
            $attributesToExport = explode(",", preg_replace('/\s+/', '', $helper->getSystemConfigValue('attributes_to_export')));
            $invalidAttributes = array_diff($attributesToExport, $this->_getExportAttrCodes());
            if(isset($invalidAttributes) && count($invalidAttributes) > 0){
                $this->_messageManager->addError(
                    __('Invalid attributes \'%1\' not found in system please check and correct it.', implode(",", $invalidAttributes)));
                throw new \Magento\Framework\Exception\LocalizedException(
                    __('Invalid attributes \'%1\' not found in system please check and correct it.', implode(",", $invalidAttributes))
                );
            } else{
                $this->_exportMainAttrCodes = array_unique(array_merge($this->_exportMainAttrCodes,$attributesToExport));
            }
        }
            ...
            foreach (array_keys($this->collectedMultiselectsData[$storeId][$productId]) as $attrKey) {
                if (!empty($this->collectedMultiselectsData[$storeId][$productId][$attrKey])) {
                    $dataRow[$attrKey] = implode(
                        ($helper->getSystemConfigValue('enable') ? "|":Import::DEFAULT_GLOBAL_MULTI_VALUE_SEPARATOR),
